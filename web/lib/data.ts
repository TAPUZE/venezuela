...
// In mock mode, auto-drafted cases produced by completed in-app intakes are merged in
// (most-recent first) so the attorney dashboard sees them alongside the sample data.

export async function listClients(): Promise<Client[]> {
  const db = createServiceClient();
  if (!db) return [...listDraftClients(), ...MOCK_CLIENTS];
...
export async function getCase(caseId: string): Promise<CaseFile | null> {
  const db = createServiceClient();
  if (!db) return getDraftCase(caseId) ?? MOCK_CASES.find((c) => c.id === caseId) ?? MOCK_CASES[0] ?? null;
...
export async function listCases(): Promise<CaseFile[]> {
  const db = createServiceClient();
  if (!db) return [...listDraftCases(), ...MOCK_CASES];
...
export async function listEvidenceForCase(caseId: string): Promise<Evidence[]> {
  const db = createServiceClient();
  if (!db) {
    const drafted = listDraftEvidence(caseId);
    return drafted.length > 0 ? drafted : MOCK_EVIDENCE.filter((e) => e.case_id === caseId);
  }
